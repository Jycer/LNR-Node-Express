...
function upload(event) {
  ...
  if (event.id == "loadCSVfile") {
    ...
    $.ajax({
      ...
      url: urlName +'/loadCSVFile', // Post the url to the server
      processData: false,
      contentType: false,
    ...
    $.ajax({
      type: 'POST', // HTTP POST command
      url: urlName +'/loadPlantUMLFile', // post url to the seruver
      processData: false,
      contentType: false,
      data: new FormData($("#uploadPUMLForm")[0]), // post selected PlantUML file
      success: function (returnData) {
        // The CSV file is uploaded to the server successfully
...
function refreshUMLSequenceChart(strSelectedFileName) {
  ...
  if (fileLoadStatus == fileLoadMode.loadCSVFile) {
    ...
    if (document.getElementById("flexRadioOriginal").checked == true) {
      ...
      $("#imgUMLSquenceChart").attr("src", "/static/generated_files/UML_sequence_charts/" + strSelectedFileNameWithouExt + "_originalUMLSequenceChart.svg?" + Math.random());
      $("#imgUMLSquenceChart").show();
    }
    else {
      // "Apply LNR algorithm" radio button is checked
      // Display processed UML sequence chart
      $("#imgUMLSquenceChart").attr("src", "/static/generated_files/UML_sequence_charts/" + strSelectedFileNameWithouExt + "_processedUMLSequenceChart.svg?" + Math.random());
      $("#imgUMLSquenceChart").show();
    }
  }
  else {
    // Display UML sequence chart for an existing PlantUML script
    $("#imgUMLSquenceChart").attr("src", "/static/generated_files/UML_sequence_charts/" + strSelectedFileNameWithouExt + "_UMLSequenceChart.svg?" + Math.random());
    $("#imgUMLSquenceChart").show();
...
/* callback function: wehn the user click the demo button, call load_samples_from_server function
 input
    - none
 return:
    - None
...
function refreshSampleSequenceChart(strSelectedFileName) {
  ...
  if (document.getElementById("flexRadioOriginal").checked == true) {
    // "original" radio button is checked
    $("#imgUMLSquenceChart").attr("src", "/static/sample_data/generated_files/UML_sequence_charts/" + strSelectedFileNameWithouExt + "_originalUMLSequenceChart.svg?" + Math.random());
    $("#imgUMLSquenceChart").show();
  }
  else {
    // "Apply LNR algorithm" radio button is checked
    // Display processed UML sequence chart
    $("#imgUMLSquenceChart").attr("src", "/static/sample_data/generated_files/UML_sequence_charts/" + strSelectedFileNameWithouExt + "_processedUMLSequenceChart.svg?" + Math.random());
    $("#imgUMLSquenceChart").show();
  }
