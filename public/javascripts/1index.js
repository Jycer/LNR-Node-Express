...
function upload(event) {
  ...
  if (event.id == "loadCSVfile") {
    ...
    $.ajax({
      ...
      url: '/loadCSVFile', // Post the url to the server
      processData: false,
      contentType: false,
    ...
    $.ajax({
      type: 'POST', // HTTP POST command
      url: '/loadPlantUMLFile', // post url to the seruver
      processData: false,
      contentType: false,
...
function imgUMLSquenceChartOnload() {
  ...
  if (fileLoadStatus == fileLoadMode.loadCSVFile) {
    ...
    if (!downloadStatus) {   // Download generated files
      let popoutOriginal = window.open('/downloadOriginalPlantUMLFile/' + strSelectedFileNameWithoutPath);
      window.setTimeout(function () {
        popoutOriginal.close();
      }, 1000);

      let popoutProcessed = window.open('/downloadProcessedPlantUMLFile/' + strSelectedFileNameWithoutPath);
      window.setTimeout(function () {
...
function DownLoadFilesclick() {
  if (fileLoadStatus == fileLoadMode.loadCSVFile) {
    // User selected a CSV file
    if (!downloadStatus) {   // Download generated files
      window.open('/downloadOriginalPlantUMLFile/' + strSelectedFileNameWithoutPath);
      window.open('/downloadProcessedPlantUMLFile/' + strSelectedFileNameWithoutPath);
...
function load_samples_from_server() {
  $.get("/samplefiles", (data, status) => {
    ...
    $("li").click(function () {
      ...
      fileLoadStatus = fileLoadMode.loadCSVFile
      $.get("/loadServerCSV/" + filename, (returnData) => {
        console.log("call");
